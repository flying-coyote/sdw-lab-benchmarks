...
| 3 | `13-reader-answer-equivalence.png` | Byte-identical 10M-row / 814-row-group Parquet (the structure behind the 100M-row C3 failure), 24 count(*) WHERE cells per reader checked against ground truth: 10 of 12 readers agree, chDB 4.1.8's v3 reader is silently wron | essay:trustworthy/verify-the-answer + web:/research + book:ch09-query-engine-selection |
| 3 | `answer-equivalence-donut.png` | 10-of-12 readers agree, KPI donut (true-share slice labels 83.3%/16.7%) | (verify-the-answer essay — not yet written) |
...
| 19 | `chdb-parquet-equality-silent-undercount.png` | chDB 4.1.8's Parquet equality path silently drops 6 to 12 matching rows on 6 of 8 probe values (52 short overall) with no error, on the 10M-row / 814-row-group repro of the 100M-row C3 failure, while DuckDB, chDB LIKE and chD | essay:trustworthy/verify-the-answer + essay:economics/how-to-run-a-benchmark-that-doesnt-lie |
