...
## Front-engine-over-Iceberg addendum — withheld

A fourth arm — a commercial Java-vectorized SQL engine querying the *same* pyiceberg-written
Iceberg files in place (byte-identical, no rewrite, same OpenSearch foil and 5-query suite,
CV-gated, answer-equality verified) — was measured here on 2026-06-14. Its per-query and
average latencies are **withheld under that engine's benchmark-publication terms**; only the
ClickHouse-over-Iceberg, ClickHouse-native, and OpenSearch-foil results above are published.

What is reportable without quoting that engine's numbers: the open-in-place pattern reproduces
the same two-regime split the flagship found (the columnar engine wins the heavy
hunting-shaped aggregations; the schema-on-read index keeps winning the cheap indexed
lookups), and the engine matters as much as the format — two engines reading byte-identical
open Iceberg files do not perform equally, which is the within-open-format counterpart to the
flagship's native-vs-Iceberg tax. The accelerated-materialization ("MV-like") path for that
engine did not produce a usable number on this read path: a materialized acceleration over a
catalog-*versioned* table registered byte-identically expires immediately (born already-stale,
`available_until` = epoch 0, never reaching an accelerable state), so the accelerated arm would
run un-accelerated. That is an integration finding about the versioned read path, not a
performance result.
