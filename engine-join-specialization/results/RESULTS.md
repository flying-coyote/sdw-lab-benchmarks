...
the DuckDB ground-truth oracle across the non-Dremio arms (43 of 44 cells; the 44th is a
loud timeout DNF, not a wrong answer). (Dremio ran as a fifth arm; its results are
withheld under its benchmark-publication terms.)**
...
(reflections OFF by design; Dremio's measured results are withheld under its
benchmark-publication terms) — all reading the same Nessie-cataloged Iceberg tables on
MinIO, byte-identical files. TPC-H disclaimer: F1 is *derived from* TPC-H (DuckDB dbgen
SF10); not audited, not comparable to published TPC-H results.
...
| query | starrocks | ch_iceberg | ch_native | trino |
|---|---|---|---|---|
| q3 (3-table) | **0.990 s** | 1.504 s | 1.364 s | 3.300 s |
| q5 (6-table) | **1.178 s** | 1.347 s | DNF | 2.453 s |
| q9 (6-table, heaviest) | **1.863 s** | 5.217 s | 5.041 s | 4.011 s |
| q18 (agg-join) | 1.893 s | 1.485 s | **1.186 s** | 6.345 s |
| q21 (correlated EXISTS) | **1.288 s** | 4.257 s | 1.908 s | 6.740 s |
| **sum** | **7.21 s** | 13.81 s | (DNF) | 22.85 s |
...
join (q9) StarRocks is 2.15× faster than Trino and 2.7–2.8× than both ClickHouse arms.
Family sums: StarRocks 1.9× faster than ClickHouse-Iceberg and 3.2× than Trino.

## F2 — SOC join suite (medians of 7)

| query | starrocks | ch_iceberg | ch_native | trino |
|---|---|---|---|---|
| j1 dim-enrichment (10M × 14.8k) | 0.167 s | 0.215 s | **0.126 s** | 0.564 s |
| j2 large-large (53.0M pairs) | 0.856 s | 0.874 s | 0.862 s | 1.411 s |
| j3 two-streams agg join | 0.100 s | **0.083 s** | **0.086 s** | 0.231 s |
| j4 IOC semi-join | 0.124 s | 0.140 s | **0.069 s** | 0.286 s |
...
StarRocks and both ClickHouse arms all pay essentially the same ~1.8× tax, and Trino is
the outlier at 4.35×. In absolute terms the tax is 77 ms on StarRocks (88 → 165 ms) — at
this scale the join is not a reason to denormalize on the non-Trino arms.

## Predictions scorecard (pre-registered, committed before any scored run)

1. **SOC ordering SR > CH ≈ Trino > Dremio (~55%/~65%): partly right** (the Dremio leg
   of this prediction is withheld under its benchmark-publication terms). ch_native took
   j1 and j4 outright, so "StarRocks first" holds on F1, not F2; among the Iceberg-reading
   arms StarRocks leads where the gate passes.
...
5. **Compressed spread (~75%): confirmed, and it is the headline.** Across 11 join
   queries and the four non-Dremio arms, every completed median is 0.069–6.74 s; the SOC
   suite never exceeds 1.5 s on any engine; the F1 family spread is 3.2× end to end
   (StarRocks vs Trino sums). The 3–26× of the vendor marketing literature (and the
   Tier-C "30–120 s vs 5–30 s" anchor the public assets carry) describes a different
   scale, not this one.
6. **Join tax (~55%): wrong.** StarRocks and both ClickHouse arms cluster at ~1.8×;
   Trino is the 4.35× outlier.
...
3. **The join tax does not justify flattening on this hardware for StarRocks or either
   ClickHouse arm** (~1.8×, sub-second absolute), and Trino's 4.35× is the measured
   version of why federation engines get handed the pre-joined copies.
4. **Dremio ran with Reflections OFF by design** — the bench deliberately did not enable
   the materialization layer that is Dremio's acceleration story, and ran it on the v3
   job-API serving path analysts actually use. Dremio's measured latencies are withheld
   under its benchmark-publication terms. The Reflections-vs-StarRocks-MV rewrite tier
   remains unmeasured by anyone, including us (named follow-up).
