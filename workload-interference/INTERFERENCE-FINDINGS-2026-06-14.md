...
hand-written headline + prediction scorecard. (The Dremio arm was run as one of the six; its
measured results are withheld here under its benchmark-publication terms, so the tables below
report the non-Dremio arms.)
...
The server engines that passed the null-load calibration gate absorb the scheduled:ad-hoc
...
well under 1.0 even at 4× demand (four arms passed the gate; the three non-Dremio arms are
reported, the Dremio arm withheld under its benchmark-publication terms):
...
| dremio | PASS | _(Dremio arm withheld under its benchmark-publication terms)_ | — | — |
...
| P4 (~55%) | every server arm sustains ≥60 scheduled queries/min before the knee | **CONFIRMED** — all 4 scored server arms sustained the full ladder to 4× with no knee (per-arm sustained-rate figures for the three non-Dremio arms; the Dremio arm withheld under its benchmark-publication terms). |
...
The Dremio arm was on this extended ladder too; its knee/p95 results are withheld under its
benchmark-publication terms, so the table below reports the three non-Dremio arms.
...
| dremio | _(Dremio arm withheld under its benchmark-publication terms)_ | — | — | — |

So on a single host the scheduled:ad-hoc mix tolerates a large multiple of the base scheduled
rate before the interactive experience inflects, and even at **64×** the probe p95 for these arms
(3–5 s) never crosses the 30 s "broken" threshold. The headroom is enormous relative to any
realistic SOC scheduled load. The ClickHouse arms and StarRocks hold flat to 32× and inflect at
64×, where the open-loop scheduler saturates (util ≥ 1.0, backlog growth → the "hard-break"
classification, not a p95 crossing). This resolves the P1/P3/P6/P7 predictions that were
inconclusive at ≤4×: a knee exists, it is far out (32–64× for these arms), and at least two arms
fail by scheduler-saturation rather than graceful degradation at that point.
...
The Dremio arm was reproduced 3× alongside these; its knee/p95 results are withheld under its
benchmark-publication terms, so the table reports the three non-Dremio arms.
...
So the single-host scheduled:ad-hoc mix tolerates **~64× the base scheduled rate** for these
arms before the interactive experience inflects, and the knee location is stable across three
runs. The p95 *magnitude* at the knee spreads run-to-run (e.g. ch_iceberg 1.16–4.05 s at 64×) —
that is expected near saturation and is why the location, not the magnitude, is the claim. (Caveat:
